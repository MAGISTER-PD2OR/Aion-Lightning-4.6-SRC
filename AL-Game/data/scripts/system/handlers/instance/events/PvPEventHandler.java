...
//        не надо если спавн отключен при создании инста
    ...
    public void onEnterInstance(Player player) {
        ...
        this.sendSpecMessage(EventManager, "Time Remaining to start 1st-Round: " + this.waitingTime + " sec.");
    ...
    public boolean isEnemy(Player attacker, Player target) {
        if (attacker != target || attacker.getRace() == target.getRace() || attacker.getRace() != target.getRace()) {
            return true;
        }
    ...
    private synchronized void StartRoundTask() {
        if (endRoundTask == null) {
            ...
            sendSpecMessage(EventManager, "Round: " + round + " - Started");
        ...
        if (timeIsUp) {
            ...
            if (winner != null) {
                ...
                if (hasWinner()) {
                    ...
                    this.sendSpecMessage(EventManager, "The round is over, by the decision of the InsaneJudge the winner for this round : " + winner.getName());
                }
            } else {
                this.DoReward();
                return;
    ...
    protected void deathPlayer(Player victim, Creature lastAttacker) {
        if (lastAttacker.getActingCreature() instanceof Player && victim != lastAttacker) {
            ...
            this.sendSpecMessage(EventManager, "Round : " + round + " Completed, Winner is... : " + winner.getName());
    ...
    protected void DoReward() {
        if (!eventIsComplete) {
            ...
            if (!isDraw) {
                ...
                Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Event: %s Completed, Winner: %s, Loser: %s", eType.getEventTemplate().getEventName(), names[0], names[1]));
            } else {
                ...
                Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Event: %s Completed, Round draw, Better luck next Time.", eType.getEventTemplate().getEventName()));
            }
