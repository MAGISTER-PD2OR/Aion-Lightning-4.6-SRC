...
public class CommandEventHandler extends BaseEventHandler {
    ...
    public void onEnterInstance(Player player) {
        ...
        if(player.isInGroup2()){
            Collection<Player> pInGroup = player.getPlayerGroup2().getMembers();
            for(Player p : pInGroup){
                if(!(p.isInPkMode())){
                    p.setInPkMode(true);
                }
            }
    ...
    protected void deathPlayer(Player vic, Creature lastAttacker) {
        if (lastAttacker.getActingCreature() instanceof Player && vic != lastAttacker) {
            ...
            if (this.containsPlayer(win.getObjectId()) && this.containsPlayer(vic.getObjectId()) && vic.isInGroup2() && win.isInGroup2()) {
        ...
        if (!this.eventIsComplete) {
            ...
            if (winner == null) {
                ...
                if (winner != null) {
                    ...
                    Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Event: %s Completed, Round Draw, Better luck Next Time.", this.eType.getEventTemplate().getEventName()));
                    for (Player p : this.players) {
                        this.moveToEntry(p);
                    }
                    return;
            ...
            Balalaka.sayInWorldOrangeTextCenter(EventManager, String.format("Event: %s Completed, The winner is the group with leader : %s.",
