    ...
    protected void deathPlayer(Player victim, Creature lastAttacker) {
        if (lastAttacker.getActingCreature() instanceof Player && victim != lastAttacker) {
            ...
            if (!hasFb) {
                ...
                hasFb = true;
                this.sendSpecMessage(EventManager, String.format("%s took his First blood by killing %s", winner.getName(), victim.getName()));
            }
            winnerScore.incPoints(winnerPoints);
            loserScore.incPoints(-loserPoints);
            PacketSendUtility.sendMessage(winner, String.format("You've received %s points.", winnerPoints));
            PacketSendUtility.sendMessage(victim, String.format("You've Lost %s points.", loserPoints));
    ...
    protected void DoReward() {
        if (!eventIsComplete) {
            ...
            for (EventScore es : this.score) {
                ...
                this.sendSpecMessage(EventManager, String.format("You took the place of %s", rank), player);
                EventRewardHelper.GiveRewardFor(player, eType, es, rank);
                this.moveToEntry(player);
                rank++;
            }

            Balalaka.sayInWorldOrangeTextCenterWithDelay(EventManager, String.format("Event: %s Completed, %s took first place on Event!!", eType.getEventTemplate().getEventName(), this.getPlayerFromEventList(this.score.get(0).PlayerObjectId).getName()), 3);
