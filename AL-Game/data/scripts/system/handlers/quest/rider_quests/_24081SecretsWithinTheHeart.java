...
public class _24081SecretsWithinTheHeart extends QuestHandler {
    ...
    public void register() {
        ...
        qe.registerQuestNpc(702093).addOnTalkEvent(questId); //Gravity Fault
    ...
    public boolean onLvlUpEvent(QuestEnv env) {
        return defaultOnLvlUpEvent(env, 24080, true);
    }
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            ...
            if (var == 2) {
                ...
                if (targetId == 218767) {
                    ...
                    QuestService.addNewSpawn(600030000, player.getInstanceId(), 702093, npc.getX(), npc.getY(), npc.getZ(),
                            (byte) 0);
                    return true;
                }
            } else if (var == 3) {
    ...
    public boolean onDialogEvent(QuestEnv env) {
        ...
        if (qs.getStatus() == QuestStatus.START) {
            switch (targetId) {
                case 802059: { //Protector Oriata
                    switch (dialog) {
                        ...
                        case SETPRO1: {
                            giveQuestItem(env, 182215406, 1);
                            return defaultCloseDialog(env, 0, 1);
                        }
                    }
                    break;
                }
                case 702093: { //Gravity Fault
                    switch (dialog) {
                        case USE_OBJECT: {
                            if (var == 2) {
                                giveQuestItem(env, 182215407, 1);
                                Npc npc = (Npc) env.getVisibleObject();
            ...
            if (targetId == 802059) {
                removeQuestItem(env, 182215407, 1);
    ...
    public HandlerResult onItemUseEvent(final QuestEnv env, Item item) {
        ...
        QuestState qs = player.getQuestStateList().getQuestState(questId);
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            if (player.isInsideZone(ZoneName.get("LDF4B_ItemUseArea_Q20062A")) && item.getItemId() == 182215406) {
