...
*/
        ...
        if(admin.getAccessLevel() < 3){
            if (DataManager.ITEM_DATA.getItemTemplate(oldItemId).getItemSlot() != DataManager.ITEM_DATA.getItemTemplate(newItemId).getItemSlot()) {
                PacketSendUtility.sendMessage(admin, "You can't :D , Go and ask a GM if you want to remodel like that, Remember it will cost couple of GP's :D");
                return;
            }
        }else{
    ...
    public void reskin(final Player admin, final int itemId, final List<Item> items) {
      //  final long gp = admin.getAbyssRank().getGp();
        RequestResponseHandler responseHandler = new RequestResponseHandler(admin) {
            @Override
            public void acceptRequest(Creature p2, Player p) {
              //  if (gp < toll) {
              //      PacketSendUtility.sendMessage(admin, "You don't have enought Vote Points (" + gp + "). You need : " + toll + " Vote Points.");
               //     return;
              //  }
        ...
        if (requested) {
            PacketSendUtility.sendPacket(admin, new SM_QUESTION_WINDOW(902247, 0, 0, "Are you sure you want to remodel this item? O.O"));
        }
    }
