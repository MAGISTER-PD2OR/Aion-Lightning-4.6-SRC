 ...
 * @author f14shm4n
 */
public class event extends PlayerCommand {
    ...
    public void execute(Player player, String... params) {
        ...
        if(params.length < 1){
            PacketSendUtility.sendMessage(player, "You can register to 1vs1 event by typing '.event reg pvp' !");
            return;
        }

        if(!params[1].equals("pvp")){
            PacketSendUtility.sendMessage(player, "Only PvP (1vs1) is allowed till it fixes!");
           return;
        }
        if (params.length == 2) {
