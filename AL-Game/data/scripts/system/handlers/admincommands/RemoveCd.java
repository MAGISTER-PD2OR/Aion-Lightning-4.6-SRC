        ...
        if (target instanceof Player) {
            ...
            if (params.length == 0) {
                ...
                while(ita.hasNext()){
                    ...
                    if(admin.getWorldId() == player1.getWorldId()){
                        if (player1.getSkillCoolDowns() != null) {
                            long currentTime = System.currentTimeMillis();
                            for (Entry<Integer, Long> en : player1.getSkillCoolDowns().entrySet()) {
                        ...
                        if (player1.getItemCoolDowns() != null) {
                            for (Entry<Integer, ItemCooldown> en : player1.getItemCoolDowns().entrySet()) {
                                delayIds.add(en.getKey());
                            }

                            for (Integer delayId : delayIds) {

                                player1.addItemCoolDown(delayId, 0, 0);
                            }

                            delayIds.clear();
                            PacketSendUtility.sendPacket(player1, new SM_ITEM_COOLDOWN(player1.getItemCoolDowns()));
                            }
                        }
                    }
                ...
                if(admin.getAccessLevel() < 4){
                    PacketSendUtility.sendMessage(admin, "You need access level 4 or higher to use this command!");
                    return;
                }
                ...
                if (player.equals(admin)) {
                    PacketSendUtility.sendMessage(admin, "Your cooldowns were removed fully.");
                } else {
                    PacketSendUtility.sendMessage(admin, "You have removed cooldowns of player: " + player.getName());
                    PacketSendUtility.sendMessage(player, "Your cooldowns were removed by admin");
                }
