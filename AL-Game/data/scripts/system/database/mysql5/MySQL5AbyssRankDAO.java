...
public class MySQL5AbyssRankDAO extends AbyssRankDAO
{
    ...
    public static final String UPDATE_PLAYER_RANK_LIST = "UPDATE abyss_rank SET abyss_rank.old_rank_pos = abyss_rank.rank_pos, abyss_rank.rank_pos = @a:=@a+1 where player_id in (SELECT id FROM players where race = ?) order by ap desc" + (GSConfig.ABYSSRANKING_SMALL_CACHE ? " limit 500" : "");
...
<<<<<<< .mine
    ...
    @Override
    public Map<Integer, Integer> loadPlayersGp(final Race race, final int lowerGpLimit) {
        final Map<Integer, Integer> results = new HashMap<Integer, Integer>();
        DB.select(SELECT_GP_PLAYER, new ParamReadStH() {
            @Override
            public void handleRead(ResultSet rs) throws SQLException {
                while (rs.next()) {
                    int playerId = rs.getInt("player_id");
	...
	public ArrayList<AbyssRankingResult> getAbyssRankingPlayers(final Race race) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (playerClass == null)
					continue;
				AbyssRankingResult rsl = new AbyssRankingResult(name, playerAbyssRank, playerId, ap, gp, playerTitle, playerClass, playerLevel, playerLegion,
						oldRankPos, rankPos);
				results.add(rsl);
			}
			resultSet.close();
