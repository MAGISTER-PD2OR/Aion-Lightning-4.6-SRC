...
public class PvPSpreeService {
    ...
    public static void increaseRawKillCount(Player winner) {
        ...
        PacketSendUtility.sendBrightYellowMessage(winner, LanguageHandler.translate(CustomMessageId.KILL_COUNT) + newRawKillCount);
    ...
    private static void sendUpdateSpreeMessage(Player winner, int level) {
        for (Player p : World.getInstance().getAllPlayers()) {
            ...
            if (level == 3) {
                PacketSendUtility.sendBrightYellowMessageOnCenter(p, winner.getName() + LanguageHandler.translate(CustomMessageId.CUSTOM_MSG1) + winner.getCommonData().getRace().toString().toLowerCase() + LanguageHandler.translate(CustomMessageId.MSG_SPREE3) + STRING_SPREE3 + LanguageHandler.translate(CustomMessageId.MSG_SPREE3_1));
            }
