...
public class VeteranRewardsService {
    ...
    public void VerifyVeteranReward(int id, String Player,int typeID, int itemID, int countID, int kinahID, String sender, String title, String message, RecipientType recipientType) {
        String recipient = null;
    ...
    private void SendVeteranRewardMail(String sender, String recipientName, String title, String message, int attachedItemObjId, int attachedItemCount, int attachedKinahCount, int mailtype) {
	    if (attachedItemObjId != 0) {
	        ...
	        if (itemTemplate == null) {
	        	if(VeteranRewardConfig.VETERANREWARDS_ENABLED_ERROR_LOG) {
	        		log.error("[SenderName: " + sender + "] [RecipientName: " + recipientName + "] RETURN ITEM ID:" + itemTemplate + " ITEM COUNT " + attachedItemCount + " KINAH COUNT " + attachedKinahCount + " ITEM TEMPLATE IS MISSING ");
				}
	            return;
	        }
	    }
	    ...
	    if (recipientName.length() > 16) {
	    	if(VeteranRewardConfig.VETERANREWARDS_ENABLED_ERROR_LOG) {
	    		log.error("[SenderName: " + sender + "] [RecipientName: " + recipientName + "] ITEM RETURN" + attachedItemObjId + " ITEM COUNT " + attachedItemCount + " KINAH COUNT " + attachedKinahCount + " RECIPIENT NAME LENGTH > 16 ");
			}
	        return;
	    }
	    ...
	    if (sender.length() > 16) {
	    	if(VeteranRewardConfig.VETERANREWARDS_ENABLED_ERROR_LOG) {
	    		log.error("[SenderName: " + sender + "] [RecipientName: " + recipientName + "] ITEM RETURN" + attachedItemObjId + " ITEM COUNT " + attachedItemCount + " KINAH COUNT " + attachedKinahCount + " SENDER NAME LENGTH > 16 ");
			}
	        return;
	    }
	    ...
	    if (recipientCommonData.isOnline()) {
	        if (!onlineRecipient.getMailbox().haveFreeSlots()) {
	        	if(VeteranRewardConfig.VETERANREWARDS_ENABLED_ERROR_LOG) {
	        		log.error("[SenderName: " + sender + "] [RecipientName: " + onlineRecipient.getName() + "] ITEM RETURN" + attachedItemObjId + " ITEM COUNT " + attachedItemCount + " KINAH COUNT " + attachedKinahCount + " MAILBOX FULL ");
				}
	            return;
	        }
	    } else {
	        if (recipientCommonData.getMailboxLetters() >= 100) {
	        	if(VeteranRewardConfig.VETERANREWARDS_ENABLED_ERROR_LOG) {
	        		log.error("[SenderName: " + sender + "] [RecipientName: " + recipientName + "] ITEM RETURN " + attachedItemObjId + " ITEM COUNT " + attachedItemCount + " KINAH COUNT " + attachedKinahCount + " MAILBOX FULL ");
				}
	            return;
	        }
	    ...
	    Letter newLetter = new Letter(IDFactory.getInstance().nextId(), recipientCommonData.getPlayerObjId(), attachedItem, finalAttachedKinahCount, title, message, finalSender, time, true, type);
		...
		if(VeteranRewardConfig.VETERANREWARDS_ENABLED_INFO_LOG) {
			log.info(String.format("Player: " + recipientName + " / " + "Item: " + attachedItemObjId + " / " + "Item Count: " + attachedItemCount + " / " + "Kinah: " + attachedKinahCount + " / " + "Status: successfully."));
		}
    }
