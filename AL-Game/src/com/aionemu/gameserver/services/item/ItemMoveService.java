...
public class ItemMoveService {
    ...
    public static void switchItemsInStorages(Player player, byte sourceStorageType, int sourceItemObjId, byte replaceStorageType, int replaceItemObjId) {
        ...
        //ItemTemplate template = DataManager.ITEM_DATA.getItemTemplate(sourceItemObjId);

        Item sourceItem = sourceStorage.getItemByObjId(sourceItemObjId);
        if (sourceItem == null) {
            return;
        }
        /*log.info("Switch en cours  : " + template.getWeaponType() + " " + sourceStorageType + " " + sourceItemObjId + " " + replaceStorageType + " " + replaceItemObjId);
         if (template.getWeaponType() == WeaponType.KEYBLADE_2H) {
         if (player.isRobotForm()) {
         PacketSendUtility.broadcastPacket(player, new SM_RIDE_ROBOT(player, 0), true);
         player.setRobotForm(false);
         }
         }*/
