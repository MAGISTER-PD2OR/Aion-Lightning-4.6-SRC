...
>>>>>>> .r274
    ...
    public void see(VisibleObject object) {
        ...
        if (object instanceof Player) {
            ...
            PacketSendUtility.sendPacket(getOwner(), new SM_EMOTION_SWITCH(npc));
...
<<<<<<< .mine
        if (master instanceof Npc || master == player) {
            if (player.getLevel() > 4 && player.getBattleGround() == null && !isNoDeathPenaltyInEffect()) {
...
<<<<<<< .mine
        ...
        if (player.battlegroundFlag != null) {
            player.getBattleGround().broadcastToBattleGround(player.getCommonData().getName() + (player.getLegion() != null ? " de " + player.getLegion().getLegionName() : "") + " got the " + (player.battlegroundFlag.getRace() == Race.ELYOS ? "Elys̩en" : "Asmodien") + " !", null);
            if (master instanceof Player) {
