...
<<<<<<< .mine
  ...
  private FastMap<Integer, List<InstanceCooltime>> syncIdToInstances = new FastMap();
  private FastMap<Integer, InstanceCooltime> instanceIdToCooltime = new FastMap();
=======
	@XmlElement(name = "instance_cooltime", required = true)
	protected List<InstanceCooltime> instanceCooltime;
    ...
    for (InstanceCooltime tmp : this.instanceCooltime)
    {
      ...
      if ((tmp.getCount() > 0) && (tmp.getSyncId() > 0))
      {
        if (!this.syncIdToInstances.containsKey(Integer.valueOf(tmp.getSyncId()))) {
          this.syncIdToInstances.put(Integer.valueOf(tmp.getSyncId()), new ArrayList());
        }
        ((List)this.syncIdToInstances.get(Integer.valueOf(tmp.getSyncId()))).add(tmp);
      }
    }
