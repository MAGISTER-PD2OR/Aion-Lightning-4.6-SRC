...
<<<<<<< .mine
		...
		if (EternityConfig.CRAFT_NPC_SPAWN_ENABLE) {
			AdvancedCraftSpawn.getInstance().spawnNpcs();
		}
		if (EternityConfig.INVASION_RIFT_ENABLE) {
			CapitalInvadeService.getInstance();
		}
		if (EternityConfig.INVASION_ENABLED) {
			BalaurInvadeService.getInstance();
		}
		if (EternityConfig.TIMEAPENABLE) {
			TimeApService.getInstance().start();
		}
		if (EternityConfig.BATTLEGROUNDS_ENABLED) {
			BattleGroundManager.initialize();
		}
		if (EternityConfig.FFA_ENABLE) {
			FFaService.getInstance().announceTask(15);
            //BattleGroundService.getBGService().announceForBGRegistering(30);
        }
...
>>>>>>> .r274
		...
		log.info("==================================================");
<<<<<<< .mine
		log.info("===========>Aion lightning Core,developed by InsaneAion (G-Group)<==============");
=======
