...
public class Item extends AionObject implements IExpirable, StatOwner {
    ...
    private int fusionedspecialCount = 0;
    ...
    public boolean isCanPolish() {
        return this.itemTemplate.isCanPolish();
    }
  
    public int getSpecialSlots() {
        return this.itemTemplate.getSpecialSlots();
    }

    public void setHasSpecialCount(boolean add, boolean isFus) {
        if (add) {
            if (!isFus) {
                this.specialCount += 1;
            } else {
                this.fusionedspecialCount += 1;
            }
        }
        else if (!isFus) {
            this.specialCount -= 1;
    ...
    public void clearSpecialCount(boolean isFus) {
        if (!isFus) {
            this.specialCount = 0;
        } else {
            this.fusionedspecialCount = 0;
        }
    }

    public boolean SpecialIsFull(boolean isFus) {
        if (!isFus) {
            return this.specialCount >= getItemTemplate().getSpecialSlots();
        }
        return this.fusionedspecialCount >= getFusionedItemTemplate().getSpecialSlots();
    }
    ...
    public GodStone addGodStone(int itemId) {
        ...
        godStone = new GodStone(getObjectId().intValue(), itemId, state);
        return godStone;
    }
    ...
    public void expireMessage(Player player, int time) {
        if (player != null) {
            PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1400481, new Object[] { new DescriptionId(getNameId()), Integer.valueOf(time) }));
        }
    }
