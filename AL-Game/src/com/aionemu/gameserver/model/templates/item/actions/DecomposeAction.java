...
=======
    chunkSand = new HashMap<Race, int[]>();
>>>>>>> .r274
...
=======
  private ExtractedItemsCollection selectItemByChance(Collection<ExtractedItemsCollection> itemsCollections) {
    ...
    float rnd = Rnd.get(0, (int)(sumOfChances - 1.0F) * 1000) / 1000.0F;
...
public void act(final Player player, final Item parentItem, final Item targetItem) {
    ...
    {
      SelectItems selectitem = DataManager.DECOMPOSABLE_SELECT_ITEM_DATA.getSelectItem(player.getPlayerClass(), parentItem.getItemId());
      PacketSendUtility.sendPacket(player, new SM_SELECT_ITEM_LIST(selectitem, parentItem.getObjectId().intValue()));
      return;
    }
    List<ExtractedItemsCollection> itemsCollections = DataManager.DECOMPOSABLE_ITEMS_DATA.getInfoByItemId(parentItem.getItemId());
>>>>>>> .r274
...
=======
    PacketSendUtility.broadcastPacketAndReceive(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId().intValue(), parentItem.getObjectId().intValue(), parentItem.getItemId(), 3000, 0, 0));
	...
	public void abort()
      {
        ...
        PacketSendUtility.broadcastPacket(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId().intValue(), parentItem.getObjectId().intValue(), parentItem.getItemTemplate().getTemplateId(), 0, 2, 0), true);
>>>>>>> .r274
    ...
    player.getObserveController().attach(observer);player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable()
    {
      @Override
	public void run()
      {
            ...
            for (RandomItem randomItem : selectedCollection.getRandomItems())
            {
              ...
              if (randomType != null)
              {
				...
				case MANASTONE_LEGEND_GRADE_70:
                  if (DecomposeAction.manastones == null) {
                    manastones = DataManager.ITEM_DATA.getManastones();
                  }
                  List<ItemTemplate> stones = DecomposeAction.manastones.get(Integer.valueOf(randomType.equals(RandomType.MANASTONE) ? itemLvl : randomType.getLevel()));
                  if (stones == null)
                  {
                  ...
                  if (!randomType.equals(RandomType.MANASTONE))
                  {
                    ...
                    {
                      if (randomType.name().contains("LEGEND")) {
                        itemQuality = ItemQuality.LEGEND;
                      }
                    }
                    List<ItemTemplate> selectedStones = Lambda.select(stones, Lambda.having(Lambda.on(ItemTemplate.class).getItemQuality(), Matchers.equalTo(itemQuality)));
                    randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
                  }
                  else
                  {
                    List<ItemTemplate> selectedStones = Lambda.select(stones, Lambda.having(Lambda.on(ItemTemplate.class).getItemQuality(), Matchers.not(Matchers.equalTo(ItemQuality.LEGEND))));
                    randomId = selectedStones.get(Rnd.get(selectedStones.size())).getTemplateId();
                  }
                  ...
                  break;
                case CHUNK_EARTH:
                  int[] earth = DecomposeAction.chunkEarth.get(player.getRace());
...
=======
                  ...
                  break;
                case CHUNK_SAND:
                  int[] sand = DecomposeAction.chunkSand.get(player.getRace());
>>>>>>> .r274
...
=======
                ...
                case CHUNK_GEMSTONE:
                  ...
                  break;
                case SCROLLS:
                  randomId = DecomposeAction.scrolls[Rnd.get(DecomposeAction.scrolls.length)];
                  if (!ItemService.checkRandomTemplate(randomId))
                  {
                    log.warn("DecomposeAction random item id not found. " + randomId);
                  }
                  break;
                case POTION:
                  randomId = DecomposeAction.potion[Rnd.get(DecomposeAction.potion.length)];
                  if (!ItemService.checkRandomTemplate(randomId))
                  {
                    log.warn("DecomposeAction random item id not found. " + randomId);
                  }
        ...
        PacketSendUtility.broadcastPacketAndReceive(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId().intValue(), parentItem.getObjectId().intValue(), parentItem.getItemId(), 0, validAction ? 1 : 2, 0));
      }
>>>>>>> .r274
