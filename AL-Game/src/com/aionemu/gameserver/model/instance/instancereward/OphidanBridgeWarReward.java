 ...
 * @author Eloann
 */
public class OphidanBridgeWarReward extends InstanceReward<OphidanBridgeWarPlayerReward> {
    ...
    private Race race;
    private FastList<OphidanBridgeWarGroupReward> groups = new FastList<OphidanBridgeWarGroupReward>();
    ...
    public OphidanBridgeWarReward(Integer mapId, int instanceId, WorldMapInstance instance) {
        super(mapId, instanceId);
        this.instance = instance;
        winnerPoints = mapId == 301210000 ? 3000 : 4500;
        looserPoints = mapId == 301210000 ? 1500 : 2500;
        drawPoins = mapId == 301210000 ? 2250 : 3750;
        bonusTime = 12000;
        buffId = 11;
        setStartPositions();
    }
    ...
    public OphidanBridgeWarGroupReward getOphidanBridgeWarGroupReward(Integer object) {
        for (InstancePlayerReward reward : groups) {
            OphidanBridgeWarGroupReward ophidanBridgeWarGroupReward = (OphidanBridgeWarGroupReward) reward;
            if (ophidanBridgeWarGroupReward.containPlayer(object)) {
                return ophidanBridgeWarGroupReward;
            }
        }
        return null;
    }

    public FastList<Player> getPlayersInside(OphidanBridgeWarGroupReward group) {
    ...
    public void addOphidanBridgeWarGroup(OphidanBridgeWarGroupReward reward) {
        groups.add(reward);
    }

    public FastList<OphidanBridgeWarGroupReward> getGroups() {
        return groups;
    }

    public List<OphidanBridgeWarPlayerReward> sortPoints() {
    ...
    public void regPlayerReward(Integer object) {
        if (!containPlayer(object)) {
            addPlayerReward(new OphidanBridgeWarPlayerReward(object, bonusTime, buffId));
        }
    }

    @Override
    public void addPlayerReward(OphidanBridgeWarPlayerReward reward) {
        super.addPlayerReward(reward);
    }

    @Override
    public OphidanBridgeWarPlayerReward getPlayerReward(Integer object) {
        return (OphidanBridgeWarPlayerReward) super.getPlayerReward(object);
    }
