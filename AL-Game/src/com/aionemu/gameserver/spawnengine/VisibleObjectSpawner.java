...
public class VisibleObjectSpawner {
    ...
    protected static NpcTemplate RatedTemplate(int objId) {
        ...
        if (!com.aionemu.gameserver.GameServer.npcs_count.contains(String.valueOf(objId))) {
                ...
                switch (npcTemplate.getRating()) {
                    case JUNK:
                        RateHP = RateConfig.JUNK_MOBS_RATE_HP;
                        RatePW = RateConfig.JUNK_MOBS_RATE_PW;
                        break;
                    case NORMAL:
    ...
    protected static VisibleObject spawnNpc(SpawnTemplate spawn, int instanceIndex) {
        ...
        Npc npc = new Npc(iDFactory.nextId(), new NpcController(), spawn, RatedTemplate(objectId));
    ...
    public static SummonedHouseNpc spawnHouseNpc(SpawnTemplate spawn, int instanceIndex, House creator, String masterName) {
        ...
        if (template == null) {
            log.error("No template for House NPC " + String.valueOf(npcId));
            return null;
        }

        SummonedHouseNpc npc = new SummonedHouseNpc(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(npcId), creator, masterName);
    ...
    protected static VisibleObject spawnRiftNpc(RiftSpawnTemplate spawn, int instanceIndex) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Rift NPC " + String.valueOf(objectId));
            return null;
        }
        IDFactory iDFactory = IDFactory.getInstance();
        Npc npc;
        ...
        if (loc.isOpened() && spawnId == loc.getId()) {
            npc = new Npc(iDFactory.nextId(), new NpcController(), spawn, RatedTemplate(objectId));
            npc.setKnownlist(new NpcKnownList(npc));
        } else {
            return null;
        }
    ...
    protected static VisibleObject spawnSiegeNpc(SiegeSpawnTemplate spawn, int instanceIndex) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Siege NPC " + String.valueOf(objectId));
            return null;
        }
        IDFactory iDFactory = IDFactory.getInstance();
        Npc npc = null;
        ...
        if ((spawn.isPeace() || loc.isVulnerable()) && spawnSiegeId == loc.getLocationId() && spawn.getSiegeRace() == loc.getRace()) {
            // default: GUARD
            npc = new SiegeNpc(iDFactory.nextId(), new NpcController(), spawn, RatedTemplate(objectId));
            npc.setKnownlist(new NpcKnownList(npc));
    ...
    protected static VisibleObject spawnInvasionNpc(VortexSpawnTemplate spawn, int instanceIndex) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Invasion NPC " + String.valueOf(objectId));
            return null;
        }
        IDFactory iDFactory = IDFactory.getInstance();
        Npc npc;
    ...
    public static Trap spawnTrap(SpawnTemplate spawn, int instanceIndex, Creature creator, int skillId) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Trap NPC " + String.valueOf(objectId));
            return null;
        }

        Trap trap = new Trap(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(objectId));
    ...
    public static GroupGate spawnGroupGate(SpawnTemplate spawn, int instanceIndex, Creature creator) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Group Gate NPC " + String.valueOf(objectId));
            return null;
        }
       
        GroupGate groupgate = new GroupGate(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(objectId));
    ...
    public static Kisk spawnKisk(SpawnTemplate spawn, int instanceIndex, Player creator) {
        ...
        NpcTemplate template = DataManager.NPC_DATA.getNpcTemplate(npcId);
        if (template == null) {
            log.error("No template for Kisk NPC " + String.valueOf(npcId));
            return null;
        }
       
        Kisk kisk = new Kisk(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(npcId), creator);
    ...
    public static Npc spawnPostman(final Player owner) {
        ...
        NpcTemplate template = DataManager.NPC_DATA.getNpcTemplate(npcId);
        if (template == null) {
            log.error("No template for Postman NPC " + String.valueOf(npcId));
            return null;
        }
        ...
        final Npc postman = new Npc(iDFactory.nextId(), new NpcController(), spawn, RatedTemplate(npcId));
    ...
    public static Npc spawnFunctionalNpc(final Player owner, int npcId, SummonOwner summonOwner) {
        NpcTemplate template = DataManager.NPC_DATA.getNpcTemplate(npcId);
        if (template == null) {
            log.error("No template for Functional NPC " + String.valueOf(npcId));
            return null;
        }
        ...
        Vector3f pos = GeoService.getInstance().getClosestCollision(owner,
                ...
                owner.getZ(), false, CollisionIntention.PHYSICAL.getId());
        SpawnTemplate spawn = SpawnEngine.addNewSingleTimeSpawn(worldId, npcId, pos.getX(), pos.getY(), pos.getZ(), (byte) 0);
        final Npc functionalNpc = new Npc(iDFactory.nextId(), new NpcController(), spawn, RatedTemplate(npcId));
    ...
    public static Servant spawnServant(SpawnTemplate spawn, int instanceIndex, Creature creator, int skillId, int level, NpcObjectType objectType) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Servant NPC " + String.valueOf(objectId));
            return null;
        }
        ...
        Servant servant = new Servant(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(objectId), servantLevel);
    ...
    public static Servant spawnEnemyServant(SpawnTemplate spawn, int instanceIndex, Creature creator, byte servantLvl) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Enemy Servant NPC " + String.valueOf(objectId));
            return null;
        }
       
        Servant servant = new Servant(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(objectId), servantLvl);
        servant.setKnownlist(new NpcKnownList(servant));
        servant.setEffectController(new EffectController(servant));
        servant.setCreator(creator);
    ...
    public static Homing spawnHoming(SpawnTemplate spawn, int instanceIndex, Creature creator, int attackCount, int skillId, int level, int homingSkillId) {
        ...
        NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
        if (npcTemplate == null) {
            log.error("No template for Homing NPC " + String.valueOf(objectId));
            return null;
        }
        ...
        Homing homing = new Homing(IDFactory.getInstance().nextId(), new NpcController(), spawn, RatedTemplate(objectId), homingLevel, skillId);
    ...
    public static Summon spawnSummon(Player creator, int npcId, int skillId, int skillLevel, int time) {
        ...
        if (npcTemplate == null) {
            log.error("No template for Summon NPC " + String.valueOf(npcId));
            return null;
        }
        ...
        Summon summon = new Summon(IDFactory.getInstance().nextId(), isSiegeWeapon ? new SiegeWeaponController(npcId) : new SummonController(), spawn, RatedTemplate(npcId), isSiegeWeapon ? npcTemplate.getLevel() : level, time);
    ...
    public static Pet spawnPet(Player player, int petId) {
        ...
        if (petCommonData == null) {
            log.error("No CommonData for Pet " + String.valueOf(petId));
            return null;
        }
        PetTemplate petTemplate = DataManager.PET_DATA.getPetTemplate(petId);
        if (petTemplate == null) {
                log.error("No template for Pet " + String.valueOf(petId));
                return null;
        }
